<?php 
$apikey = "fe2ac8fb54e8cf2079a8b30f35cf261b";
$weatherflowID = "5153";
$weatherflowdairID = "14584";
$weatherflowdskyID = "14847";
$weatherflowoption   = "no";
$weatherflowmapzoom   = "5";
$id = "I34KAGIT3";
$TZ = "Europe/London";
$UTC = "0";
$lon = 0.9784;
$lat = 50.0082;
$stationlocation = "KEN TRUE";
$stationName = "KEN TRUE CUMULUS DEMO";
$moonadj = "0";
$minmag = "3";
$unit = "metric";
$metric = true;
$elevation = "120ft or 45m";

$purpleairID = "12194";
$purpleairhardware   = "no";
$darkskyunit   = "ca";

$indoor = true;
$uk = true;
$usa = false;
$scandinavia = false;
$restoftheworld = false;
$windunit = "mph";
$distanceunit = "mi";
$tempunit = "F";
$rainunit  = "in";
$rainrate = "/h";
$pressureunit  = "inHg";
$livedataFormat = "cumulus";
$livedata   = "demodata/realtime.txt";

$currentconditions   = "currentconditionsmetar34.php";
$boltekfile   = "demodata/NSRealtime.txt";



$personalmessage   = "You can include a personalised footer description.. try and keep it short doesn't need to be long.";
$extralinks   = "no";
$languages   = "yes";

$dateFormat   = "d-m-Y";
$timeFormat    = "g:i:s a";
$timeFormatShort    = "g:i a";
$clockformat    = "24";

$showDate = false;
$fireriskshow = false;
$position1   = "weather34clock.php";
$position2   = "max-minwind.php";
$position3   = "rainfallf-year-month.php";
$position4   = "advisory.php";
$position1title   = "Weather Station <orange> Time";
$position2title   = "Wind | Gust <oblue> Data";
$position3title   = "Rainfall <oblue> Data";
$position4title   = "Weather <ored>Advisory</ored>";
$temperaturemodule   = "temperaturein.php";
$hardware   = "add your hardware";
$email    = "Something@dot.com";
$twitter   = "@weather34dotcom";
$showEqNotDaylight   = false;
$notificationeq   = "no";
$uvhardware    = "solaruvds.php";
$theme1   = "dark";
$since    = "Jan 2030";

$db_host   = "localhost";
$db_user    = "root";
$db_pass  = "";
$db_name   = "weatherstation";

$metarapikey ="you require an API key ";

$metar   = "no";
$icao1   = "LTBA";
$airport1   = "Istanbul,Turkey";
$airport1dist   = "6";

$notifications = "yes";
$sunoption = "sun2.php";
$weatherhardware   = "Davis® Vantage Pro2™";
$davis   = "Yes";
$sunpositionbearing = "75";
$cloudbase   = "feet";
$hemisphere   = "0";

$defaultlanguage   = "en";
$language    = "en";
$password    = "";
$https    = "yes";
$flag   = "en";
?>