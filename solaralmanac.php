...
actual{font-size:13px;float:right;position:absolute;left:125px;top:20px;-webkit-border-radius:4px;-moz-border-radius:4px;-o-border-radius:4px;border-radius:4px;background:rgba(61, 64, 66, 1.000);padding:5px;}.uvmaxi3{position:absolute;left:-30px;color:rgba(0, 154, 171, 1.000);margin-top:-40px;font-size:16px;width:240px;}.uvmaxi3 span{color:#aaa}sup{font-size:10px;}.higust{color:#aaa;font-size:12px;}.higust span{color:rgba(255, 124, 57, 1.000)}
windgusts{position:absolute;font-size:15px;margin-top:20px;left:50px;color:#444}.hitempy{background:rgba(61, 64, 66, 0.8);color:#aaa;font-size:12px;width:220px;padding:3px;border-radius:4px;margin-top:0;margin-left:5px;}
  ...
  <section class="weather34cards">
   <div class="weather34card weather34card--earthquake1">
            ...
            <div class="weather34card_weather34-wrapper"><span class="weather34card__count-text--big">
              <?php
	// UV INDEX
	if ($weather["solardmax"]>900)  {
	echo "<red>",$weather["solardmax"] . "</value>";} 	
	else if ($weather["solardmax"]>100)  {
	echo "<orange>",$weather["solardmax"] . "</value>";} 		
	else if ($weather["solardmax"]>=0) {
	echo "<green>",$weather["solardmax"] . "</value>";}		
	echo "<sup><unit> W/m<sup>2</sup> </span>"
?>
...
<path fill="<?php if($weather["solardmax"]>0){echo " #ff8841";} else echo "currentcolor"?>"  opacity="1.00" d=" M 7.00 72.00 C 17.00 72.00 27.00 72.00 37.00 72.00 C 37.00 73.67 37.00 75.33 37.00 77.00 C 27.00 77.00 17.00 77.00 7.00 77.00 C 7.00 75.33 7.00 73.67 7.00 72.00 Z" /></svg>
 ...
 <br>
<div class="hitempy">  
  <?php if ($weather["solardmax"] < $weather["solarydmax"]){echo "Max Solar <orange>W/m<sup>2</sup></orange> <green>Today</green> Recorded is <green>Less</green> than Yesterday"; echo " ",$weather["solarydmax"] - $weather["solardmax"], " <orange>W/m<sup>2</sup></orange> <green>".$fallingsymbol."</green>";}  
  else if ($weather["solardmax"] > $weather["solarydmax"]){echo "Max Solar W/m<sup>2</sup> <green>Today</green> Recorded <orange>Higher</orange> than Yesterday";echo " ",$weather["solardmax"] - $weather["solarydmax"], " <orange>W/m<sup>2</sup> ".$risingsymbol."</orange>";}
  ?></span>
            </div> 
...
<path fill="<?php if($weather["solarmmax"]>0){echo " #ff8841";} else echo "currentcolor"?>"  opacity="1.00" d=" M 7.00 72.00 C 17.00 72.00 27.00 72.00 37.00 72.00 C 37.00 73.67 37.00 75.33 37.00 77.00 C 27.00 77.00 17.00 77.00 7.00 77.00 C 7.00 75.33 7.00 73.67 7.00 72.00 Z" /></svg>
              ...
              <?php
	//month	
	if ($weather["solarmmax"]>900)  {
	echo "<red>",$weather["solarmmax"] . "</value>";} 	
	else if ($weather["solarmmax"]>100)  {
	echo "<orange>",$weather["solarmmax"] . "</value>";} 	
	else if ($weather["solarmmax"]>=0) {
	echo "<green>",$weather["solarmmax"] . "</value>";}	
	echo "<sup><unit>W/m<sup>2</sup> </span>"	
?>
...
<path fill="<?php if($weather["solarydmax"]>0){echo " #ff8841";} else echo "currentcolor"?>"  opacity="1.00" d=" M 7.00 72.00 C 17.00 72.00 27.00 72.00 37.00 72.00 C 37.00 73.67 37.00 75.33 37.00 77.00 C 27.00 77.00 17.00 77.00 7.00 77.00 C 7.00 75.33 7.00 73.67 7.00 72.00 Z" /></svg>
	...
	if ($weather["solarydmax"]>900)  {
	echo "<red>",$weather["solarydmax"] . "</value>";}	
	else if ($weather["solarydmax"]>100)  {
	echo "<orange>",$weather["solarydmax"] . "</value>";} 	
	else if ($weather["solarydmax"]>0) {
	echo "<green>",$weather["solarydmax"] . "</value>";}	
	echo "<sup><unit>W/m<sup>2</sup></span>"
...
<path fill="<?php if($weather["solarymax"]>0){echo " #ff8841";} else echo "currentcolor"?>"  opacity="1.00" d=" M 7.00 72.00 C 17.00 72.00 27.00 72.00 37.00 72.00 C 37.00 73.67 37.00 75.33 37.00 77.00 C 27.00 77.00 17.00 77.00 7.00 77.00 C 7.00 75.33 7.00 73.67 7.00 72.00 Z" /></svg>
	...
	if ($weather["solarymax"]>900)  {
	echo "<red>",$weather["solarymax"] . "</value>";} 		
	else if ($weather["solarymax"]>100)  {
	echo "<orange>",$weather["solarymax"] . "</value>";} 	
	else if ($weather["solarymax"]>=0) {
	echo "<green>",$weather["solarymax"] . "</value>";}	
	echo "<sup><unit>W/m<sup>2</sup></span>"	
?>
